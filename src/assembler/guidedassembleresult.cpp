...
//                nuclQuerySeq.mapSequence(id, queryKey, nuclQuery.c_str());
            ...
            if (queryCouldBeExtended) {
                ...
                nuclResultWriter.writeData(nuclQuery.c_str(), nuclQuery.size(), queryKey, thread_idx, true);
                aaResultWriter.writeData(aaQuery.c_str(), aaQuery.size(), queryKey, thread_idx, true);
            }
        }
    } // end parallel
...
//        bool wasNotExtended =  !(wasExtended[id] & 0x80);
        ...
        if (isNotContig){
            ...
            nuclResultWriter.writeData(querySeqData, queryLen, nuclSequenceDbr->getDbKey(id), thread_idx, false);
            ...
            aaResultWriter.writeData(queryAASeqData, queryAALen, aaSequenceDbr->getDbKey(id), thread_idx, false);
        }
    }
