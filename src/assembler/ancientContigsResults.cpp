...
public:
    bool operator() (const Matcher::result_t & r1,const Matcher::result_t & r2) {
        ...
        float mm_count1 = r1.alnLengthCons - r1.deamMatch;
        float mm_count2 = r2.alnLengthCons - r2.deamMatch;
...
#pragma omp for schedule(dynamic, 100)
        for (size_t id = 0; id < sequenceDbr->getSize(); id++) {
            ...
            for (unsigned int idx = 0; idx < alignments.size(); idx++){
                ...
                alignments[idx].alnLengthCons = alignments[idx].alnLengthCons;

                bool isNotIdentity = (queryKey != alignments[idx].dbKey);

                if ( alignments[idx].seqId >= par.mergeSeqIdThr && alignments[idx].rySeqId >= par.rySeqIdThr  && isNotIdentity){
