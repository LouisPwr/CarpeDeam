...
int createsubdb(int argc, const char **argv, const Command& command) {
    ...
    while (getline(&line, &len, orderFile) != -1) {
        ...
        if (par.subDbMode == Parameters::SUBDB_MODE_SOFT) {
            writer.writeIndexEntry(key, reader.getOffset(id), reader.getEntryLen(id), 0, reader.getExtData(id));
        } else {
            ...
            bool wasExtended = reader.getExtData(id);
            ...
            if (isCompressed) {
                ...
                writer.writeData(data, entryLength, key, 0, wasExtended, false, false);
            } else {
                writer.writeData(data, entryLength, key, 0, wasExtended, true, false);
            }
            // do not write null byte since
            writer.writeIndexEntry(key, writer.getStart(0), originalLength, 0, wasExtended);
        }
    }
