...
void copyEntry(unsigned int oldKey, unsigned int newKey, DBReader<unsigned int>& reader, DBWriter& writer, bool isCompressed, int subDbMode) {
    ...
    if (subDbMode == Parameters::SUBDB_MODE_SOFT) {
        //Louis was here
        writer.writeIndexEntry(newKey, reader.getOffset(id), reader.getEntryLen(id), 0, reader.getExtData(id));
    } else {
        ...
        writer.writeIndexEntry(newKey, writer.getStart(0), originalLength, 0, reader.getExtData(id));
    }
}
