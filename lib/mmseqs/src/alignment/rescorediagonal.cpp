...
//                                seqId = (alignment.score1 / static_cast<float>(std::max(qAlnLength, dbAlnLength)))  * 0.1656 + 0.1141;
                    ...
                    //bool hasRySeqId = rySeqId >= (par.rySeqIdThr - std::numeric_limits<float>::epsilon());
                    ...
                    if (isIdentity || hasToFilter || (hasAlnLen && hasCov && hasSeqId && hasEvalue)) {
                        if (par.rescoreMode == Parameters::RESCORE_MODE_ALIGNMENT ||
                            ...
                            // Louis was here
                            // calculating and adding rySeqId to the result
                            //rySeqId = Matcher::computeRySeqId(result, querySeqToAlign, targetSeq);
                            //result.rySeqId = rySeqId;
                            // adding wasExtendeInfo
                            //bool extended = qdbr->getExtData(targetId);
                            //result.wasExtended = extended;  
