...
Alignment::Alignment(const std::string &querySeqDB, const std::string &targetSeqDB,
        ...
        covThr(par.covThr), canCovThr(par.covThr), covMode(par.covMode), seqIdMode(par.seqIdMode), evalThr(par.evalThr), seqIdThr(par.seqIdThr), rySeqIdThr(par.rySeqIdThr),
...
#pragma omp for schedule(dynamic, 5) reduction(+: alignmentsNum, totalPassedNum)
            for (size_t id = start; id < (start + bucketSize); id++) {
                ...
                while (*data != '\0' && passedNum < maxAccept && rejected < maxReject) {
                    ...
                    // adding wasExtendeInfo
                    //bool extended = qdbr->getExtData(dbKey);
                    //res.wasExtended = extended;
                    //std::cerr << "Inside alignment cpp, res.extended = " << extended << std::endl;
                    ...
                    // Louis was here, please change
                    // std::unordered_map<char, char> nucleotideMap = {
                    //     {'A', 'T'},
                    //     {'C', 'G'},
                    //     {'G', 'C'},
                    //     {'T', 'A'}};
                    ...
                    // char * orientedSeq;
                    // char *fragmentRev = new char[tdbr->getSeqLen(dbId)+2];
                    // if ( isReverse )
                    // {
                    //     for (int pos = tdbr->getSeqLen(dbId) - 1; pos > -1; pos--) {
                    //         char revRes = nucleotideMap.at(querySeqData[pos]);
                    //         fragmentRev[(tdbr->getSeqLen(dbId) - 1) - pos] = revRes;
                    //     }
                    //     fragmentRev[tdbr->getSeqLen(dbId)] ='\0';
                    //     fragmentRev[tdbr->getSeqLen(dbId)+1] ='\n';                        
                    //     orientedSeq = fragmentRev;
                    // }
                    // else{
                    //     orientedSeq = dbSeqData;
                    // }

                    // // Louis was here
                    // // changed to checkCriteriaRy
                    // float ryId = Matcher::computeRySeqId(res, querySeqData, orientedSeq);
                    // delete [] fragmentRev;
                    // res.rySeqId = ryId;
                    // if ( Parameters::DBTYPE_NUCLEOTIDES == 0){
                    //     if (checkCriteriaRY(res, isIdentity, evalThr, seqIdThr, rySeqIdThr, alnLenThr, covMode, covThr)) {
...
bool Alignment::checkCriteriaRY(Matcher::result_t &res, bool isIdentity, double evalThr, double seqIdThr, double seqIdThrRy, int alnLenThr, int covMode, float covThr){
    const bool evalOk = (res.eval <= evalThr); // -e
    const bool seqIdOK = (res.seqId >= seqIdThr); // --min-seq-id
    const bool seqIdRyOK = (res.rySeqId >= seqIdThrRy);
    const bool covOK = Util::hasCoverage(covThr, covMode, res.qcov, res.dbcov);
    const bool alnLenOK = Util::hasAlignmentLength(alnLenThr, res.alnLength);
