...
//    alignment->score1 = alignment->score1 - log2(dbSeq->L);
    ...
    // Louis was here, adding the wasExtendedInfo
    // bool isReverseToAlignment = isReverse;
...
float Matcher::computeRySeqId(Matcher::result_t & res, char* querySeq,  char* targetSeq)
{
    std::unordered_map<char, int> mapACGT = {
    {'A', 0},
    {'C', 1},
    {'G', 0},
    {'T', 1}};
...
Matcher::result_t Matcher::parseAlignmentRecord(const char *data, bool readCompressed) {
    ...
    // Louis was here
    //std::cout << "inside alignment (Matcher.cpp)" << std::endl;
    //std::cout << "result.dbLen: " << result.dbLen << std::endl << "result.score: " << result.score << std::endl << "result.seqId: " << result.seqId << std::endl << "result.eval: " << result.eval << std::endl << "result.dbKey: " << result.dbKey << std::endl << "result.qStartPos: " << result.qStartPos << std::endl << "result.qEndPos: " << result.qEndPos  << std::endl << "result.qLen: " << result.qLen << std::endl << "result.dbStartPos: " << result.dbStartPos << std::endl << "result.dbEndPos: " << result.dbEndPos << std::endl << "result.dbLen: " << result.dbLen << std::endl;
    //std::cout << "result.seqId: " << seqId << "\t" << "result.eval: " << eval << "\t" << "result.qStartPos: " << qStart << "\t" << "result.qEndPos: " << qEnd  << "\t" << "result.qLen: " << qLen << "\t" << "result.dbStartPos: " << dbStart << "\t" << "result.dbEndPos: " << dbEnd << "\t" << "result.dbLen: " << dbLen << std::endl;
    //std::cout << std::endl;
