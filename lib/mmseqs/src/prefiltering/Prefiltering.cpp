...
#pragma omp parallel num_threads(threads)
    {
        ...
        while(currentId < reader1.getSize()){
            ...
            writer.writeData(result.c_str(), result.size(), reader1.getDbKey(currentId), thread_idx, reader1.getExtData(currentId));
            hits.clear();
            result.clear();
            prevId = currentId;
...
#pragma omp for schedule(dynamic, 2) reduction (+: kmersPerPos, resSize, dbMatches, doubleMatches, querySeqLenSum, diagonalOverflow, trancatedCounter)
        for (size_t id = queryFrom; id < queryFrom + querySize; id++) {
            ...
            // Louis was here
            bool wasExtended = qdbr->getExtData(id);
            tmpDbw.writeData(result.c_str(), result.length(), qKey, thread_idx, wasExtended);
            result.clear();
