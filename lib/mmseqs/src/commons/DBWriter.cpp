...
#pragma omp for schedule(static)
        for (size_t id = 0; id < dbr.getSize(); id++) {
            ...
            writeData(data, (length == 0 ? 0 : length - 1), dbr.getDbKey(id), thread_idx, dbr.getExtData(id));
        }
    };
...
void DBWriter::writeEnd(unsigned int key, unsigned int thrIdx, bool wasExtended, bool addNullByte, bool addIndexEntry) {
    // close stream
    bool isCompressedDB = (mode & Parameters::WRITER_COMPRESSED_MODE) != 0;
    if(isCompressedDB) {
...
// keep original size in index
        ...
        writeIndexEntry(key, starts[thrIdx], length, thrIdx, wasExtended);
    }
}

void DBWriter::writeIndexEntry(unsigned int key, size_t offset, size_t length, unsigned int thrIdx, bool wasExtended){
    char buffer[1024];
    size_t len = indexToBuffer(buffer, key, offset, length, wasExtended);
...
void DBWriter::writeData(const char *data, size_t dataSize, unsigned int key, unsigned int thrIdx, bool wasExtended, bool addNullByte, bool addIndexEntry) {
    writeStart(thrIdx);
    writeAdd(data, dataSize, thrIdx);
    writeEnd(key, thrIdx, wasExtended, addNullByte, addIndexEntry);
}

size_t DBWriter::indexToBuffer(char *buff1, unsigned int key, size_t offsetStart, size_t len, bool  wasExtended){
    char * basePos = buff1;
    ...
    *(tmpBuff-1) = '\t';
    tmpBuff = Itoa::u32toa_sse2(static_cast<uint32_t>(wasExtended), tmpBuff);
    *(tmpBuff-1) = '\n';
    *(tmpBuff) = '\0';
...
void DBWriter::writeIndexEntryToFile(FILE *outFile, char *buff1, DBReader<unsigned int>::Index &index){
    ...
    *(tmpBuff-1) = '\t';
    tmpBuff = Itoa::u32toa_sse2(static_cast<uint32_t>(index.wasExtended),tmpBuff);
    *(tmpBuff-1) = '\n';
    *(tmpBuff) = '\0';
...
void DBWriter::createRenumberedDB(const std::string& dataFile, const std::string& indexFile, const std::string& origData, const std::string& origIndex, int sortMode) {
    ...
    for (size_t i = 0; i < reader.getSize(); i++) {
        ...
        size_t len = DBWriter::indexToBuffer(buffer, i, idx->offset, idx->length, idx->wasExtended);
