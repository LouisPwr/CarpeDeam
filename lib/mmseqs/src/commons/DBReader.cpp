...
void DBReader<unsigned int>::sortIndex(bool isSortedById) {
    ...
    if ((isSortedById == false) && (accessType != HARDNOSORT) && (accessType != SORT_BY_OFFSET)) {
        ...
        for (unsigned int i = 0; i < size; i++) {
            ...
            indexAndOffsetBuff.wasExtended = index[i].wasExtended;
            ...
            while (1) {
                ...
                index[j].wasExtended = index[k].wasExtended;
                ...
                index[j].wasExtended = indexAndOffsetBuff.wasExtended;
            }
        }
        ...
        while (currPos < indexDataSize){
            ...
            if(currLine == lineStartId){
                for(size_t startIndex = lineStartId; startIndex < lineStartId + BATCH_SIZE && currPos < indexDataSize; startIndex++){
                    // Louis was here
                    Util::getWordsOfLine(indexDataChar, cols, 4);
                    ...
                    size_t extended = Util::fast_atoi<size_t>(cols[3]);
                    localDataSize += length;
                    index[startIndex].offset = offset;
                    index[startIndex].length = length;
                    index[startIndex].wasExtended = extended;
...
//Louis was here
template<typename T>
bool DBReader<T>::getExtData(size_t id) {
    if (id >= size){
        Debug(Debug::ERROR) << "Invalid database read for id=" << id << ", database index=" << indexFileName << "\n";
        Debug(Debug::ERROR) << "getExtData: local id (" << id << ") >= db size (" << size << ")\n";
        EXIT(EXIT_FAILURE);
    }
    if (local2id != NULL) {
        id = local2id[id];
    }
    return index[id].wasExtended;
}
