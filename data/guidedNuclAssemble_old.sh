...
#     NUM_IT="$((NUM_IT + PREF_NUM_IT))"
#     printf "\nUSING PREFILTER: NUM_IT = %s\n" "${PREF_NUM_IT}"
# fi
...
#     awk '$3 > $7 { print }' "${RESULT_NUCL}_tmp.index" >"${RESULT_NUCL}_only_assembled.index"
# fi
...
if notExists "${TMP_PATH}/nuclassembly.dbtype"; then
    # shellcheck disable=SC2086
    "$MMSEQS" nuclassemble "${TMP_PATH}/nucl_reads" "${TMP_PATH}/nuclassembly" "${TMP_PATH}/nuclassembly_tmp" ${NUCL_ASM_PAR}
    # "${TMP_PATH}/guided_assembly.merged"
